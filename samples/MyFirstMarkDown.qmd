...
```{r question1}
head (diatask)
#The variables in diatask are cut and price.
```
...
```{r question4}
# Sparkles is expected to have 10 rows.

print(diatask, n = nrow(diatask))

premium_rows <- diatask[grep("Premium", diatask$cut), ]
ideal_rows <- diatask[grep("Ideal", diatask$cut), ]

sparkles <- rbind(premium_rows, ideal_rows)

sparkles
nrow(sparkles)

# Yes, sparkles has 10 rows, which matches the original prediction.
```
...
# Grey gridlines removed by adding theme_classic().
```
...
```{r question9}
# The diatask and diasize plots only use 21 rows from the full diamonds so a conclusion about the real relationships between cut and price or carat and price can not be made.The rows were also selected by position, instead of randomly, so they may not represent the full dataset. Price can also be influenced/related to other variables, such as color, clarity, cut and carat.
```
...
# All rows from the diamonds dataset were used. The carat and price variables were also selected. The main reason why the full dataset was use because using the full dataset gives much stronger evidence about the relationship between the variables than using only 21 rows. I found that diamonds with larger carat values generally have higher prices. I was not surprised by the overall relationship, but I was surprised that diamonds with similar carat values can still have very different prices. This suggests that other diamond characteristics also affect price such as color and clarity of the diamonds.
